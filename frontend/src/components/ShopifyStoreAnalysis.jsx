...
function ShopifyStoreAnalysis({ clientId, clientData }) {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex justify-between items-start">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <h2 className="text-xl font-bold text-white">{store.store_name || 'Shopify Store'}</h2>
                  <p className="text-gray-400 text-sm">{store.store_url}</p>
                </div>
              </div>
              {store.last_sync_at && (
            ...
            <button
              ...
              disabled={analyzing}
              className="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 disabled:bg-gray-600 disabled:cursor-not-allowed"
            >
      ...
      {showConnectForm && (
        <div className="bg-gray-800 rounded-lg p-6">
          <h3 className="text-xl font-bold text-white mb-4">Connect Shopify Store</h3>
          <form onSubmit={handleConnect} className="space-y-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2 bg-gray-700 text-white rounded-lg border ${
                  formErrors.store_url ? 'border-red-500' : 'border-gray-600'
                }`}
              />
              {formErrors.store_url && (
                <p className="mt-1 text-sm text-red-400">{formErrors.store_url}</p>
              )}
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2 bg-gray-700 text-white rounded-lg border ${
                  formErrors.access_token ? 'border-red-500' : 'border-gray-600'
                }`}
              />
              {formErrors.access_token && (
                <p className="mt-1 text-sm text-red-400">{formErrors.access_token}</p>
              )}
              <p className="mt-1 text-xs text-gray-500">
      ...
      {analyzing && (
        <div className="bg-blue-900/50 border border-blue-700 rounded-lg p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-lg font-semibold text-blue-300">Analyzing Store...</h3>
      ...
      {analysis && (
        <div className="space-y-6">
          {/* Store Summary */}
          <div className="bg-gray-800 rounded-lg p-6">
            <h3 className="text-xl font-bold text-white mb-4">Store Analysis Summary</h3>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-gray-700/50 p-4 rounded-lg">
                <p className="text-sm text-gray-400 mb-1">SEO Score</p>
                <p className="text-3xl font-bold text-white">{analysis.seo_score || 0}</p>
                <p className="text-xs text-gray-500 mt-1">out of 100</p>
              </div>
              <div className="bg-gray-700/50 p-4 rounded-lg">
                <p className="text-sm text-gray-400 mb-1">Products Analyzed</p>
                <p className="text-3xl font-bold text-white">{analysis.store_summary?.total_products || 0}</p>
              </div>
              <div className="bg-gray-700/50 p-4 rounded-lg">
                <p className="text-sm text-gray-400 mb-1">Collections</p>
                <p className="text-3xl font-bold text-white">{analysis.store_summary?.total_collections || 0}</p>
              </div>
              <div className="bg-gray-700/50 p-4 rounded-lg">
                <p className="text-sm text-gray-400 mb-1">Recommendations</p>
                <p className="text-3xl font-bold text-white">{analysis.overall_recommendations?.length || 0}</p>
              </div>
            </div>
          ...
          {analysis.product_analyses && analysis.product_analyses.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              <h3 className="text-xl font-bold text-white mb-4">Product SEO Analysis</h3>
                    ...
                    <div key={idx} className="bg-gray-700/50 p-4 rounded-lg">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex-1">
                          <h4 className="font-semibold text-white">{product.product_title}</h4>
                          {product.product_url && (
                            <a
                        ...
                        <div className="text-right">
                          <div className="text-2xl font-bold text-white">{product.seo_score || 0}</div>
                      ...
                      {product.title_recommendation && (
                        <div className="mt-3 p-3 bg-gray-600/30 rounded">
                          <p className="text-xs text-gray-400 mb-1">Title Optimization</p>
                      ...
                      {product.keyword_opportunities && product.keyword_opportunities.length > 0 && (
                        <div className="mt-3">
                          <p className="text-xs text-gray-400 mb-2">Keyword Opportunities</p>
                          <div className="flex flex-wrap gap-2">
                            {product.keyword_opportunities.slice(0, 3).map((kw, kwIdx) => (
                              <div
                                key={kwIdx}
                                className="bg-blue-900/30 px-3 py-1 rounded text-sm"
                              >
                                <span className="text-blue-300 font-medium">{kw.keyword}</span>
                                {kw.search_volume && (
                                ...
                                {kw.cpc && (
                                  <span className="text-green-400 text-xs ml-2">${kw.cpc}</span>
                                )}
                              </div>
                            ))}
                          </div>
          ...
          {analysis.sales_opportunities && analysis.sales_opportunities.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              <h3 className="text-xl font-bold text-white mb-4">💰 Sales Opportunities</h3>
              <div className="space-y-3">
                {analysis.sales_opportunities.slice(0, 10).map((opp, idx) => (
                  <div key={idx} className="bg-green-900/20 border border-green-700/30 p-4 rounded-lg">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <h4 className="font-semibold text-green-300 mb-1">{opp.product_title}</h4>
                        <p className="text-gray-300 text-sm mb-2">{opp.recommendation}</p>
                        {opp.keyword && (
                          <div className="flex items-center gap-2 text-sm">
                            <span className="text-gray-400">Keyword:</span>
                            <span className="text-white font-medium">{opp.keyword}</span>
                            {opp.cpc && (
                              <span className="text-green-400">CPC: ${opp.cpc}</span>
                            )}
                            {opp.search_volume && (
              ...
              <div className="space-y-2">
                {analyses.map((a) => (
                  <button
                    ...
                    className="w-full text-left bg-gray-700/50 p-4 rounded-lg hover:bg-gray-700 transition-colors"
                  >
