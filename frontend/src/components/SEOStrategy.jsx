            ...
            {selectedStrategy && (
              <div className="bg-gray-800 rounded-lg p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <h4 className="text-white font-semibold mb-2">Primary Keywords</h4>
                  ...
                  <div>
                    <h4 className="text-white font-semibold mb-2">Secondary Keywords</h4>
                    <div className="flex flex-wrap gap-2">
                ...
                {selectedStrategy.content_pillars && selectedStrategy.content_pillars.length > 0 && (
                  <div>
                    <h3 className="text-xl font-bold text-white mb-3">Content Pillars</h3>
                    <div className="space-y-4">
                      {selectedStrategy.content_pillars.map((pillar, idx) => (
                        <div key={idx} className="bg-gray-700 rounded-lg p-4">
                          <h4 className="text-white font-semibold mb-2">{pillar.theme}</h4>
                          ...
                          {pillar.goals && (
                            <p className="text-gray-300 text-sm">{pillar.goals}</p>
                          )}
                        </div>
                      ))}
                    </div>
                ...
                {selectedStrategy.technical_seo_recommendations && (
                  <div>
                    <h3 className="text-xl font-bold text-white mb-3">Technical SEO Recommendations</h3>
                    ...
                    <div className="space-y-3">
                      {selectedStrategy.content_calendar.map((month, idx) => (
                        <div key={idx} className="bg-gray-700 rounded-lg p-4">
                          ...
                          <p className="text-gray-300 text-sm mb-2">Theme: {month.theme}</p>
                          {month.priority_pieces && month.priority_pieces.length > 0 && (
                            <div>
                              <p className="text-gray-400 text-xs mb-1">Priority Pieces:</p>
                ...
                {selectedStrategy.link_building_opportunities && selectedStrategy.link_building_opportunities.length > 0 && (
                  <div>
                    <h3 className="text-xl font-bold text-white mb-3">Link Building Opportunities</h3>
                    <div className="space-y-2">
                      {selectedStrategy.link_building_opportunities.map((opp, idx) => (
                        <div key={idx} className="bg-gray-700 rounded-lg p-3 flex justify-between items-start">
                          <div>
                ...
                {selectedStrategy.content_gap_analysis && selectedStrategy.content_gap_analysis.length > 0 && (
                  <div>
                    <h3 className="text-xl font-bold text-white mb-3">Content Gap Analysis</h3>
                    <div className="space-y-2">
                ...
                {selectedStrategy.competitor_gaps && selectedStrategy.competitor_gaps.length > 0 && (
                  <div>
                    <h3 className="text-xl font-bold text-white mb-3">Competitor Content Gaps</h3>
                    <div className="space-y-2">
                      {selectedStrategy.competitor_gaps.map((gap, idx) => (
                        <div key={idx} className="bg-gray-700 rounded-lg p-3">
                          <div className="flex justify-between items-start mb-1">
                            <h4 className="text-white font-medium">{gap.opportunity}</h4>
                            <span className={`px-2 py-1 rounded text-xs ${
                              gap.priority === 'high' ? 'bg-red-600 text-white' :
                              gap.priority === 'medium' ? 'bg-yellow-600 text-white' :
                              'bg-gray-600 text-gray-300'
                ...
                {selectedStrategy.competitor_analysis_summary && (
                  <div>
                    <h3 className="text-xl font-bold text-white mb-3">Competitor Analysis Summary</h3>
                    <div className="bg-gray-700 rounded-lg p-4">
                      <p className="text-gray-300 whitespace-pre-line">
                        {selectedStrategy.competitor_analysis_summary}
                      </p>
